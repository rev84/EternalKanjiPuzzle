...
define('CONSUMER_KEY', 'PvLmFXdgrS2DWsWMNmIg');
define('CONSUMER_SECRET', 'cn52MTRL6jFSguSCmukwIrIb0zIhreuZGNRkgXsY');
define('ACCESS_TOKEN', '17474646-pTiVD3enAvrUqcesQ9NjoG4lr6VecPO25EAEPVMsB');
define('ACCESS_TOKEN_SECRET', 'Dfz2Uh3ira2OtvWD2d1BcnW1m3aVHKw3BGURtGJuzIlCA');
...
$sql = 'select * from words where twitter_count is null order by rand() ';
...
foreach ($gen as $r) {
    ...
    if ($html !== false) {
      ...
      // 件数
      $res = preg_match('`<div class="sd" id="resultStats">([^<]+)</div>`is', $html, $matches);

      // 件数あり
      if ($res) {
          $count = preg_replace('`[^\d]`', '', $matches[1]);
          $pdo = new PDOMain();
          $sql = 'update words set google_count = ? where char1 = ? and char2 = ? ';
